...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, cbam_head=False, cbam_tail=False, selfAttention_head=False, selfAttention_middle=False, double_fc=False, dropout=False, fpn=False):
        super(ResNet, self).__init__()
        self.cbam_head = cbam_head
        self.cbam_tail = cbam_tail
        self.selfAttention_head = selfAttention_head
        self.selfAttention_middle = selfAttention_middle
        ...
        if self.double_fc:
            if self.fpn:
                self.fc1 = nn.Linear(2048+1024+512, 1024)
    ...
    def forward(self, x):
        ...
        if self.fpn:
            ...
            x2 = self.layer2(x1)
            out_2 = self.avgpool(x2)
            out_2 = torch.flatten(out_2, 1)

            x3 = self.layer3(x2)
            out_3 = self.avgpool(x3)
            out_3 = torch.flatten(out_3, 1)

            x4 = self.layer4(x3)
            out_4 = self.avgpool(x4)
            out_4 = torch.flatten(out_4, 1)

            x = torch.cat([out_2, out_3, out_4], dim=1)
        else:
            x = self.layer1(x)
            x = self.layer2(x)
            x = self.layer3(x)
